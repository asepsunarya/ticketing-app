<template>
  <ui-modal id="upload-icon-modal" size="2xs">
    <div class="font-semibold text-xl mb-3">Pilih Ikon</div>
    <div class="flex flex-col gap-y-3 items-center justify-center">
      <div
        class="flex flex-col justify-center items-center rounded-full w-56 h-56 border-2 border-dashed p-6"
      >
        <img
          v-if="selectedIcon"
          class="w-24 h-20 rounded-lg"
          :src="selectedIcon"
          alt=""
        />
        <img
      ...
      <div class="flex gap-x-2 mt-2">
        <img
          ...
          :key="index"
          :src="icon"
          :class="{ 'border-2 p-[1px] border-primary': icon === selectedIcon }"
    ...
    <div class="flex justify-end items-center gap-3 mt-8">
      <ui-button text="Batal" size="sm" type="ghost" for="upload-icon-modal" />
      <ui-button
...
import { projectIcons } from "@/views/admin/projects/constants/project-icons.const";
...
function setIcon() {
  selectedIcon.value = projectIcons[0];
}

function handleSelect() {
  emits("selected", selectedIcon.value);
  closeModal("upload-icon-modal");
}
